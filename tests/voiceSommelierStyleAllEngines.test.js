...
async function run() {
    ...
    // This is the exact failure mode this test exists to catch: an admin
    // replacing the persona text via the dashboard's custom-prompt config
    // must NOT be able to silently drop the voice style, since
    // buildRealtimeSystemInstruction() injects it unconditionally,
    // independent of where the `persona` argument came from.
    ...
    try {
        ...
        assert.ok(geminiSession.systemInstructionText.includes('ОДНИМ словом'), 'Gemini Live session text must carry the one-word-answer rule');
    } finally {
    ...
    try {
        ...
        assert.ok(grokSession.systemInstructionText.includes('ОДНИМ словом'), 'Grok Realtime session text must carry the one-word-answer rule');
    } finally {
    ...
    try {
        ...
        assert.ok(classicSession.systemInstructionText.includes('ОДНИМ словом'), 'Classic session text must carry the one-word-answer rule');
    } finally {
