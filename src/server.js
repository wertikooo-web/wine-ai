...
async function handleRequest(req, res) {
    ...
    // itself always comes from here. When the structured Wine.md catalog is
    // available and has a product for this wine card / entity, its product
    // URL and price (with last_synced_at freshness) win over the static
    // fixture, so live catalog data is the source of truth for buy links.
    ...
    if (req.method === 'GET' && purchaseOptionsMatch) {
        ...
        const catalogRows = db.isEnabled()
            ? await catalogStore.findCatalogProductsById(wineId).catch(() => [])
            : [];
        const catalogOptions = catalogRows.map((product) => ({
            id: `catalog_${product.external_id || product.id}`,
            wineId,
            sellerName: 'Wine.md',
            sellerType: 'marketplace',
            url: product.product_url || product.image_url || null,
            price: product.price == null ? null : Number(product.price),
            currency: product.currency || 'MDL',
            country: 'MD',
            image_url: product.image_url || null,
            availability: product.availability || 'unknown',
            synced_at: product.last_synced_at ? (product.last_synced_at.toISOString ? product.last_synced_at.toISOString() : product.last_synced_at) : null,
        })).filter((option) => option.url);
        // Source-of-truth invariant (Phase 3): when the Wine.md catalog is
        // available, price/stock/availability/photo/product URL/buy link come
        // ONLY from it -- an empty catalog result stays empty, it must never
        // silently resolve to the static demo fixture in production. The
        // fixture is only a dev/test fallback for when no Postgres catalog
        // exists at all.
        if (catalogOptions.length) {
            return sendJson(res, 200, { ok: true, wine_id: wineId, source: 'catalog', options: catalogOptions });
        }
        if (db.isEnabled()) {
            return sendJson(res, 200, { ok: true, wine_id: wineId, source: 'catalog', options: [] });
        }
        return sendJson(res, 200, { ok: true, wine_id: wineId, source: 'fixture', options: getPurchaseOptions(wineId) });
    }

    // Wine.md catalog health for the admin surface (Phase 3): total / linked /
    // unmatched / stale products, last sync job state, and recent sync errors.
    if (req.method === 'GET' && pathname === '/api/catalog/status') {
        if (!db.isEnabled()) return sendJson(res, 200, { ok: true, catalog: { enabled: false } });
        try {
            const status = await catalogStore.getCatalogStatus();
            return sendJson(res, 200, { ok: true, catalog: status });
        } catch (error) {
            return sendJson(res, 500, { ok: false, error: 'catalog_status_failed', message: error.message });
        }
    }
