...
async function handleRequest(req, res) {
    ...
    // Compares every production knowledge/source/*.md chunk against PG.
    // No writes. Remove after audit.
    if (req.method === 'GET' && pathname === '/api/knowledge/reconcile') {
        const sourceDir = knowledgeLoader.DEFAULT_SOURCE_DIR;
        const { chunkText, parseFrontmatter } = knowledgeLoader;
        const crypto2 = require('crypto');
        function classifyGroup2(f) {
            if (f.startsWith('discovered-kos-')) return 'discovered-kos-*';
            if (f.startsWith('discovered-')) return 'discovered-*';
            if (f.startsWith('book-')) return 'book-*';
            if (f.startsWith('manual-')) return 'manual-*';
            if (f.startsWith('demo-')) return 'demo-*';
            if (f.startsWith('news-')) return 'news-*';
            if (f.startsWith('Ghid_')) return 'Ghid_*';
            if (f.startsWith('moldova-')) return 'moldova-*';
            if (f.startsWith('feteasca-')) return 'feteasca-*';
            return 'other-curated';
        }

        function stableId2(sourceFile, index) {
            return crypto2.createHash('sha256').update(`${sourceFile}#${index}`).digest('hex').slice(0, 16);
        }
        function computeHash2(text, sourceFile, title, chunkIndex) {
            return crypto2.createHash('sha256')
                .update('v1\n').update(String(text || ''))
                .update('\n').update(String(sourceFile || ''))
                .update('\n').update(String(title || ''))
        ...
        try {
            const files = fs.readdirSync(sourceDir).filter(f => /\.(md|txt)$/i.test(f));
            ...
            for (const file of files) {
                ...
                for (let i = 0; i < chunks.length; i++) {
                    ...
                    fsChunks.push({
                        chunkId: stableId2(file, i),
                        sourceFile: file,
                        chunkIndex: i,
                        text: chunks[i],
                        title,
                        contentHash: computeHash2(chunks[i], file, title, i),
                    });
                }
            }
            ...
            const pgResult = await pool.query(
                'SELECT chunk_id, source_file, title, text, content_hash, document_id, enabled FROM knowledge_chunks WHERE enabled = TRUE'
            );
            const pgChunks = pgResult.rows;
            const pgById = new Map(pgChunks.map(r => [r.chunk_id, r]));
            const pgByHash = new Map();
            for (const r of pgChunks) {
                if (!pgByHash.has(r.content_hash)) pgByHash.set(r.content_hash, []);
                pgByHash.get(r.content_hash).push(r);
            }

            // Classify
            const perFile = {};
            for (const chunk of fsChunks) {
                if (!perFile[chunk.sourceFile]) perFile[chunk.sourceFile] = { file: chunk.sourceFile, chunks: [], group: classifyGroup2(chunk.sourceFile) };
                const entry = { idx: chunk.chunkIndex, chunkId: chunk.chunkId, cls: '', pgDocId: null, dupOf: null, dupSource: null };
                const pgMatch = pgById.get(chunk.chunkId);
                if (pgMatch) {
                    if (pgMatch.content_hash === chunk.contentHash) {
                        entry.cls = 'exact';
                    } else {
                        entry.cls = 'changed';
                    }
                    entry.pgDocId = pgMatch.document_id;
                } else {
                    const hashMatch = pgByHash.get(chunk.contentHash);
                    if (hashMatch && hashMatch.length > 0) {
                        entry.cls = 'stale-duplicate';
                        entry.dupOf = hashMatch[0].chunk_id;
                        entry.dupSource = hashMatch[0].source_file;
                    } else {
                        entry.cls = 'fs-only';
                    }
                }
                perFile[chunk.sourceFile].chunks.push(entry);
            }

            // Summary per file
            const fileSummaries = Object.values(perFile).map(f => {
                const counts = { exact: 0, changed: 0, 'fs-only': 0, 'stale-duplicate': 0 };
                for (const c of f.chunks) counts[c.cls]++;
                return { file: f.file, group: f.group, totalChunks: f.chunks.length, ...counts };
            });

            // Group summary
            const groups = {};
            for (const fs of fileSummaries) {
                if (!groups[fs.group]) groups[fs.group] = { files: 0, chunks: 0, exact: 0, changed: 0, 'fs-only': 0, 'stale-duplicate': 0 };
                groups[fs.group].files++;
                groups[fs.group].chunks += fs.totalChunks;
                groups[fs.group].exact += fs.exact;
                groups[fs.group].changed += fs.changed;
                groups[fs.group]['fs-only'] += fs['fs-only'];
                groups[fs.group]['stale-duplicate'] += fs['stale-duplicate'];
            }

            // PG-only count
            const fsIds = new Set(fsChunks.map(c => c.chunkId));
            const pgOnlyCount = pgChunks.filter(r => !fsIds.has(r.chunk_id)).length;
            const pgOnlyByPrefix = {};
            for (const r of pgChunks) {
                if (!fsIds.has(r.chunk_id)) {
                    const p = (r.source_file || 'null').substring(0, 25);
                    pgOnlyByPrefix[p] = (pgOnlyByPrefix[p] || 0) + 1;
                }
            }

            const totalFs = fsChunks.length;
            const totalExact = fileSummaries.reduce((s, f) => s + f.exact, 0);
            const totalChanged = fileSummaries.reduce((s, f) => s + f.changed, 0);
            const totalFsOnly = fileSummaries.reduce((s, f) => s + f['fs-only'], 0);
            const totalStaleDup = fileSummaries.reduce((s, f) => s + f['stale-duplicate'], 0);
            ...
            return sendJson(res, 200, {
                ...
                production: {
                    ...
                    fsChunks: totalFs,
                    pgChunks: pgChunks.length,
                    gap: totalFs - pgChunks.length,
                },
                categories: {
                    exact: totalExact,
                    changed: totalChanged,
                    'fs-only': totalFsOnly,
                    'stale-duplicate': totalStaleDup,
                    'pg-only': pgOnlyCount,
                },
                arithmetic: {
                    fsChunks: totalFs,
                    fsInPG: totalExact + totalChanged,
                    fsNotInPG: totalFsOnly + totalStaleDup,
                    pgNotInFS: pgOnlyCount,
                    check1_totalPlusPGOnly: totalFs + pgOnlyCount,
                    check2_exactPlusChangedPlusFSOnlyPlusStalePlusPGOnly: totalExact + totalChanged + totalFsOnly + totalStaleDup + pgOnlyCount,
                    sumEquals: (totalFs + pgOnlyCount) === (totalExact + totalChanged + totalFsOnly + totalStaleDup + pgOnlyCount),
                },
                groups,
                fileSummaries: fileSummaries.sort((a, b) => b.totalChunks - a.totalChunks),
                pgOnlyByPrefix: Object.entries(pgOnlyByPrefix).sort((a, b) => b[1] - a[1]).slice(0, 30),
            });
        } catch (error) {
            return sendJson(res, 500, { ok: false, error: 'reconcile_failed', message: error.message });
        }
    }
