...
function normalizeClassicSttProvider(value) {
    return String(value || DEFAULT_STT_PROVIDER).trim().toLowerCase() === 'deepgram' ? 'deepgram' : 'whisper';
}
...
function createRealtimeProviderRegistry(config = {}, commonMetadata = {}, overrides = {}) {
    ...
    const classicSttProvider = normalizeClassicSttProvider(config.classicSttProvider || process.env.CLASSIC_STT_PROVIDER);
    const classicSttConfigured = classicSttProvider === 'deepgram' ? Boolean(deepgramKey) : Boolean(openaiKey);
    ...
    const definitions = {
        ...
        classic: {
            ...
            configured: Boolean((classicSttConfigured && geminiKey) || overrides.classic),
    ...
    function list() {
        return ['gemini', 'grok', 'classic'].map((id) => publicDefinition(definitions[id]));
    }

    function resolve(requestedProvider) {
        const id = normalizeProviderName(requestedProvider, defaultProvider);
        const definition = definitions[id];
        if (!definition) throw Object.assign(new Error('realtime_provider_unknown'), { code: 'realtime_provider_unknown' });
        if (!definition.configured) {
            throw Object.assign(new Error(`${id}_provider_not_configured`), {
                code: 'realtime_provider_not_configured', provider: id,
            });
        }
        return {
            id,
            metadata: {
                ...
                provider: id,
                model: definition.model,
                sttProvider: definition.sttProvider,
    ...
    function resolveDefault() {
        try { return resolve(defaultProvider); } catch { return resolve('mock'); }
    }
