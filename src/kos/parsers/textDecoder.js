...
/**
 * WINE AI KOS - Text Decoder Stage (Step 2A Refined)
 * Decodes raw Buffer into text with strict/lenient UTF-8 validation and BOM detection.
 */
...
function decodeText(rawBufferOrString, options = {}) {
    ...
    const warnings = [];
    // Check for replacement character (\uFFFD) representing invalid UTF-8 byte sequences
    if (rawText.includes('\uFFFD')) {
