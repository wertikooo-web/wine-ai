 ...
 * - Zero raw byte blobs in PostgreSQL (metadata only in DB, content-addressed in ObjectStorage)
 * - Child page failures produce 'partial' run status; seed failure produces 'failed'
...
async function ingestSource({
    ...
    try {
        ...
        if (queryClient) {
            await queryClient.query(
                ...
                [JSON.stringify({ code: err.code, message: err.message }), crawlRunId]
            );
        }
        throw err;
    }
    ...
    // 5. Process Successfully Fetched Resources
    ...
    // 7. Calculate Final Run Status & Counter Summary
    let finalRunStatus = crawlResult.status || 'completed';

    // If seed URL failed, mark entire run failed
    ...
    if (queryClient) {
        ...
        await queryClient.query(sqlFinal, [
            finalRunStatus,
            crawlResult.counters.discovered || 0,
            crawlResult.counters.fetched || 0,
            crawlResult.counters.failed || 0,
            completedAt,
            crawlRunId,
        ]);
    }
